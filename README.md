...
- `src/document_processing/store.cljk` — `Store` protocol +
  `MemStore`: registered document batches, committed records, an
  append-only audit ledger.
- `src/document_processing/advisor.cljk` — `Advisor` protocol;
...
- `src/document_processing/governor.cljk` —
  ...
  human sign-off.
- `src/document_processing/actor.cljk` — `build-graph`, `run-request!`,
