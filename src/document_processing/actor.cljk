(ns document-processing.actor
  "DocumentProcessingActor — the ISCO-08 4413 independent document-
  processing actor as a `langgraph.graph/state-graph` (per
  ...
  The unconditional invariant: the DocumentAdvisor can never
  directly commit a record or dispatch a robot action the
  DocumentProcessingGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [document-processing.advisor :as advisor]
            [document-processing.governor :as governor]
            [document-processing.store :as store]))

(defn build-graph
  "Build a compiled DocumentProcessingActor graph. `store` implements
  `document-processing.store/Store`. `advisor` implements
  `document-processing.advisor/Advisor` (defaults to `mock-advisor`).
