(ns document-processing.store
  "SSoT for the ISCO-08 4413 independent document-processing
  sole-proprietor actor. Store is a protocol injected into the
  `document-processing.actor` StateGraph — `MemStore` is the
  ...
  Domain:
    ...
    record   — a committed operating record under a batch (coded
               output, proofread output, confidential-document
               handling, regulated-document handling) — written ONLY
